...
CLI_TARGET="${INSTALL_BASE}/skills_store_cli.js"
UPGRADE_MODULE_TARGET="${INSTALL_BASE}/skills_upgrade.js"
PACKAGE_JSON_TARGET="${INSTALL_BASE}/package.json"
...
install_cli() {
  if ! command -v node >/dev/null 2>&1; then
    echo "Error: node is required for skillhub." >&2
    exit 1
  fi

  mkdir -p "${INSTALL_BASE}" "${BIN_DIR}"
  cp "${CLI_SRC_DIR}/skills_store_cli.js" "${CLI_TARGET}"
  cp "${CLI_SRC_DIR}/skills_upgrade.js" "${UPGRADE_MODULE_TARGET}"
  cp "${CLI_SRC_DIR}/skillhub-bin.js" "${INSTALL_BASE}/skillhub-bin.js"
  cp "${CLI_SRC_DIR}/package.json" "${PACKAGE_JSON_TARGET}"
  ...
  chmod +x "${CLI_TARGET}" "${INSTALL_BASE}/skillhub-bin.js"
  rm -rf "${INSTALL_BASE}/node_modules"
  (
    cd "${INSTALL_BASE}"
...
set_workspace_skills_preference() {
  local enabled="$1"
  if ! command -v node >/dev/null 2>&1; then
    echo "Warn: node not found; cannot persist skills preference." >&2
    return 0
  fi

  node - "$CONFIG_TARGET" "$enabled" <<'NODE'
const fs = require('fs');
...
const configPath = path.resolve(expandUser(process.argv[2]));
const enabled = String(process.argv[3] || '').trim().toLowerCase() === 'true';
const defaultUpdateUrl = 'https://skillhub-1388575217.cos.ap-guangzhou.myqcloud.com/version.json';
...
if (typeof raw.self_update_url !== 'string' || !raw.self_update_url.trim()) {
  raw.self_update_url = defaultUpdateUrl;
}
